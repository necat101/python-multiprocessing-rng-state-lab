...
def _fork_legacy_worker(q, worker_id):
    try:
        import numpy as np
        seq = np.random.RandomState().randint(LOW, HIGH, size=DRAWS).tolist()  # Wait inherited global?
        # Actually we need to use the inherited module-level legacy rng
        # We'll use global _LEGACY_RNG
        global _LEGACY_RNG
        seq = _LEGACY_RNG.randint(LOW, HIGH, size=DRAWS).tolist()
        q.put((worker_id, seq, 0))
    except Exception as e:
        try: q.put((worker_id, None, 1))
        except: pass
...
def _fork_worker_id_reseed_worker(q, worker_id):
    try:
        ...
        q.put((worker_id, seq, list(ss.entropy), ss.spawn_key if hasattr(ss, "spawn_key") else None, 0))
    except Exception:
        try: q.put((worker_id, None, None, None, 1))
        except: pass
...
def _fork_seedsequence_children_worker(q, worker_id, child_ss):
    try:
        ...
        rng = np.random.default_rng(ss)
        seq = rng.integers(LOW, HIGH, size=DRAWS, endpoint=False).tolist()
        spawn_key = ss.spawn_key if hasattr(ss, "spawn_key") else None
        q.put((worker_id, seq, spawn_key, 0))
    except Exception:
        try: q.put((worker_id, None, None, 1))
        except: pass
...
def run_workers(ctx_name, target, target_args_fn):
    try:
        ctx = multiprocessing.get_context(ctx_name)
    except Exception as e:
        return None, f"platform_skip: context {ctx_name} unavailable"
    try:
        import numpy as np
    except Exception:
...
def handle_serial_same_seed_baseline_marker(method, state):
    ...
    if method == "verify_sequence_relation":
        ...
        if all_equal and len(vals) == 4:
            return "expected_duplicate", {"all_equal": True}
...
def handle_fork_legacy_global_state_copy_marker(method, state):
    ...
    if method == "execute_workers":
        ...
        # check exit codes
        return "pass", {"worker_count": len(data["results"])}
    if method == "verify_sequence_relation":
        wd = state.get("worker_data")
        if not wd: return "fail", {"reason": "no_worker_data"}
        seqs = [wd["results"][wid][1] for wid in sorted(WORKER_IDS)]
        all_equal = all(s == seqs[0] for s in seqs)
        if all_equal:
            return "expected_duplicate", {"sequences": {wid: wd["results"][wid][1] for wid in WORKER_IDS}}
        return "fail", {"sequences": seqs}
    if method == "reproducibility_context_observation":
...
def handle_fork_worker_id_reseed_marker(method, state):
    ...
    if method == "verify_sequence_relation":
        ...
        seqs = [run1["results"][wid][1] for wid in sorted(WORKER_IDS)]
        distinct = len(set(tuple(s) for s in seqs)) == len(seqs)
        if distinct:
            return "expected_distinct", {"sequences": {wid: run1["results"][wid][1] for wid in WORKER_IDS}}
        return "fail", {"distinct": distinct}
    if method == "reproducibility_context_observation":
        run1 = state.get("run1")
        ...
        match = True
        for wid in WORKER_IDS:
            s1 = run1["results"][wid][1]
            s2 = data2["results"][wid][1]
            if s1 != s2:
                match = False
                break
        if match:
...
def handle_fork_seedsequence_children_marker(method, state):
    ...
    if method == "reproducibility_context_observation":
        ...
        if err: return "fail", {"error": err}
        match = all(run1["results"][wid][1] == data2["results"][wid][1] for wid in WORKER_IDS)
        if match:
...
def handle_spawn_same_seed_marker(method, state):
    ...
    if method == "verify_sequence_relation":
        ...
        seqs = [run["results"][wid][1] for wid in sorted(WORKER_IDS)]
        all_equal = all(s == seqs[0] for s in seqs)
        if all_equal:
            return "expected_duplicate", {"sequences": {wid: run["results"][wid][1] for wid in WORKER_IDS}}
        return "fail", {}
    if method == "reproducibility_context_observation":
...
def handle_spawn_repeatability_across_runs_marker(method, state):
    ...
    if method == "verify_sequence_relation":
        ...
        seqs1 = [r1["results"][wid][1] for wid in WORKER_IDS]
        distinct1 = len(set(tuple(s) for s in seqs1)) == len(seqs1)
        seqs2 = [r2["results"][wid][1] for wid in WORKER_IDS]
        distinct2 = len(set(tuple(s) for s in seqs2)) == len(seqs2)
        # cross-run equality per worker
        match = all(r1["results"][wid][1] == r2["results"][wid][1] for wid in WORKER_IDS)
        if distinct1 and distinct2 and match:
            return "pass", {"match": True, "distinct": True}
